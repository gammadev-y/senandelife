...
const SettingsPage: React.FC<SettingsPageProps> = ({ onWeatherPreferenceSelect }) => {
  ...
  const themeButtonClass = (theme: ThemeSetting) => 
    `flex-1 flex flex-col items-center p-3 rounded-2xl border-2 transition-all duration-300 ease-in-out
     ${currentTheme === theme 
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 md:p-8 space-y-8 text-slate-800 dark:text-slate-100">
      ...
      {error && <p className="text-red-500 bg-red-100 dark:bg-red-900/30 p-3 rounded-lg text-sm text-center animate-pulse">{error}</p>}
      ...
      <div className="p-6 bg-white dark:bg-slate-800 rounded-2xl shadow-lg">
        ...
        <div className="space-y-2 text-sm text-slate-600 dark:text-slate-300">
          ...
          <button 
            ...
            className="px-4 py-2 text-sm font-medium rounded-lg shadow-sm transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 text-white bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-500 dark:hover:bg-emerald-600 flex items-center disabled:opacity-70"
          >
